...
class Apple:
    ...
    def _add_apple_block(self, grid: np.ndarray | list) -> None:
        """Setting apple block at random place in grid
        that is not occupied by snake."""
        rect = deepcopy(self._rect)
        inside_snake = True
        while inside_snake:
            _left, _top = np.random.randint(0, self._screen_size // self._block_size, 2) * self._block_size
            if [_left, _top] not in grid:
                inside_snake = False
                rect.left, rect.top = _left, _top
        apple_block = Block(rect=rect, direction="Irrelevant")
        self.apple_block = apple_block

    def initialize(self, grid: np.ndarray | list) -> None:
...
class Snake:
    ...
    def get_tail(self):
        """ Getter function for snake tail rect."""
        return self._snake_blocks[-1].rect

    def get_body(self, index: int) -> pygame.Rect | FakeRect:
        """ Getter function for snake body rect."""
        assert index >= 1, f'First body part after head has index 1.'
    ...
    def get_grid(self):
        occupied_spots = [[_.rect.left, _.rect.top] for _ in self._snake_blocks]
